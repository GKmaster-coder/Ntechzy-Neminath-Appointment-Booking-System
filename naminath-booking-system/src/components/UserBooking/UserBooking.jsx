...
const [formData, setFormData] = useState({
  ...
  const handleDateTimeSelect = () => {
    ...
    if (available.length === 0) {
      const alternatives = findAlternativeSlots(formData.selectedDate, formData.selectedTime);
...
/>
      ...
      case 5:
        if (formData.selectedOPD && formData.paymentMethod) {
          return (
            <ConfirmationStep
              formData={formData}
              onBack={() => setStep(4)}
    ...
    <div className="min-h-screen bg-[#fff7e6] py-4 px-3 sm:py-8 sm:px-4 poppins">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-[#C00000] mb-2">Book Appointment</h1>
        </div>
