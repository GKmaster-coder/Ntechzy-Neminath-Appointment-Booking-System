...
const OPDSelectionStep = ({ formData, availableOPDs, onOPDSelect, onBack, onNext }) => {
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="text-center">
        <h3 className="text-xl sm:text-2xl font-semibold text-[#222] mb-2">Select OPD Room</h3>
        <p className="text-gray-600 text-sm sm:text-base">
          Available for <span className="font-semibold text-[#222]">{formData.selectedDate}</span> at <span className="font-semibold text-[#222]">{formData.selectedTime}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
        {availableOPDs.map(opd => (
          ...
          >
            <div className="text-lg sm:text-2xl font-bold text-[#222] mb-1 sm:mb-2">OPD {opd}</div>
      ...
      {availableOPDs.length === 0 && (
        <div className="text-center py-4">
          <p className="text-red-500 font-medium text-sm sm:text-base">No OPD rooms available for the selected time slot.</p>
        </div>
      )}
